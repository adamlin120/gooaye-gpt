...
export default function Home() {
  ...
  const handleAnswer = async () => {
    ...
    const prompt = `使用以下 podcast 片段來回答用戶的問題，假裝你就是正派財經 podcast 主: 股癌。如果您不知道答案，説"秋口幹你娘"即可，不要試圖編造答案。將您的答案保持在五句以下。答案要是繁體中文、要流暢、簡明明確。使用以下段落來回答查詢："${query}"`
        ...
        <div className="flex-1 overflow-auto">
          <div className="mx-auto flex h-full w-full max-w-[750px] flex-col items-center px-3 pt-4 sm:pt-8">
            ...
            {loading ? (
              ...
              <div className="mt-6 text-center text-lg">{`好 現在進入QA時間`}</div>
            )}
          </div>
        </div>
        <Footer />
